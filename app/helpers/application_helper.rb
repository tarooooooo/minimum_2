module ApplicationHelper
  ...
  def get_category_data(category_id, genre)
    ...
    if genre == "color"
      brand_items = current_user.items.where(category_id: category_id, item_status: "discarded").joins(:brand).order('count_all desc').group('brands.name').count.to_a
      ...
      color_items = current_user.items.where(category_id: category_id, item_status: "discarded").joins(:color).order('count_all desc').group('colors.name').count.to_a
      high_chart = LazyHighCharts::HighChart.new('graph') do |f|
        f.title(text: 'カラー別')
